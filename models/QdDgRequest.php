...
class QdDgRequest extends QdRoot
{
    ...
    protected function tagsOnValidate($field_name)
    {
        //split by comma
        $tags = explode(',', $this->{$field_name});
        $tmp = new QdMyTags();
        //check exists in MyTags
        foreach($tags as $item){
            $item = trim($item);
            $tmp->REMOVEFILTER();
            $tmp->SETRANGE('name', $item, static::$OP_EQUAL);
            if($tmp->FINDFIRST()==null){
                $tmp = new QdMyTags();
                $tmp->name = $item;
                $tmp->type = QdMyTags::$TYPE_SP;
                $tmp->save();
            }
        }
        //Insert if not exist
